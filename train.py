...
def main():
    ...
    adj = get_adjacency_matrix(distance_df_filename=args.sensors_distance,
                               ...
                               id_filename=None)  #如果是PEMS03，这里设置为'data/PEMS03/PEMS03.txt'。如果不是PEMS03,这里就是None
    #adj=torch.Tensor(adj)
    #local_adj = construct_adj(A=adj, steps=args.strides)
    #local_adj = torch.FloatTensor(local_adj)  
    adj_path='./garage/'+DATASET+'/'     
    dtw_adj=np.load(adj_path+'dtw_adj.npy')
    pearson_adj=np.load(adj_path+'pearson_adj.npy')
    st_adj=construct_st_adj(adj,dtw_adj,pearson_adj,2)
    ...
    log_string(log, "The shape of localized adjacency matrix: {}".format(adj.shape))
